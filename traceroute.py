...
def traceroute(target, dport=80, minttl=1, maxttl=30, stype="Random", srcport=50000, iface=None, l4=None, filter=None, timeout=0.005, verbose=None, gw=None, netproto="TCP", nquery=1, ptype=None, payload=b'', privaddr=0, rasn=1, **kargs):
    """A Multi-Traceroute command:
...
def usage():
  ...
  print('*** Example:')
  print('traceroute -t "www.google.com" -r 0 --timeout 1 --netproto "TCP" -p "80,443" --minttl 1 --maxttl 20 -q 2 -a "All" --rtt -v 1;\n')
...
def main(argv):
  ...
  try:
    if dotfile:
      ...
      if (verboselvl >= 1):
        print('\nNow dumping the scapy traceroute DOT Digraph (multi_traceroute._graphdef) to file: "{df:s}"'.format(df = dotdirfile))
      dg = open(dotdirfile, 'w')
      dg.write(multi_traceroute._graphdef)
      dg.close()
  except:
    print('\n**ERROR*** scapy traceroute failed to dump DOT file: "{df:s}"'.format(df = dotfile))
    sys.exit(5)
  #
  ...
  try:
    if (pcapdirfilename != ''):
      ...
      pkts = []                             # All packets
      for ans in multi_traceroute._res:                 # Dump all completed send/received packets
      ...
      pkts = spkts + rpkts
      for uans in multi_traceroute._ures:               # Dump all unanswered packets
